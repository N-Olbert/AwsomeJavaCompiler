...
public class ParseTreeGenerator {
    ...
    public UntypedProgram generate(awsomeJavaParser.ProgrammContext programContext){
        ...
        if(programContext.jClass() != null){
            for (int i=0;i<programContext.getChildCount();i++){
                String classAccessModifier =

               awsomeJavaParser.JClassContext classContext = programContext.jClass(i);
               ...
               bodyContext.fieldDeclaration().forEach(fieldDecl -> {
                   String fieldAccessModifier = fieldDecl.AccessModifier().getText();
